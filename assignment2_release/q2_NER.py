...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      #raise NotImplementedError
      embedding = tf.Variable(tf.convert_to_tensor(self.wv, dtype=tf.float32), name="Embedding")
  ...
  def add_model(self, window):
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    with tf.variable_scope('layer_tanh', initializer=xavier_weight_init()) as scope:
        self.W = tf.get_variable("W", [self.config.window_size * self.config.embed_size, self.config.hidden_size])
        self.b1 = tf.get_variable("b1", [self.config.hidden_size])
        self.W2norm = tf.reduce_sum(tf.square(self.W))
        dropped_window = tf.nn.dropout(window, self.dropout_placeholder)
        self.h = (tf.matmul(dropped_window, self.W) + self.b1)
    with tf.variable_scope("softmax", initializer=xavier_weight_init()) as scope:
        self.U = tf.get_variable("U", [self.config.hidden_size, self.config.label_size])
        self.U2norm = tf.reduce_sum(tf.square(self.U))
        self.b2 = tf.get_variable("b2", [self.config.label_size])
        #output = softmax(tf.matmul(self.h, self.U) + self.b2)
        dropped_h = tf.nn.dropout(self.h, self.dropout_placeholder)
        output = tf.matmul(dropped_h, self.U) + self.b2
    ### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, y):
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    loss_soft = tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)
    loss_reg = self.config.l2 * ( self.W2norm + self.U2norm )
    loss = tf.reduce_mean(loss_soft) + loss_reg
    ### END YOUR CODE
    return loss
