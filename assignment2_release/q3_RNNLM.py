...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      inputs0 = tf.split(1, self.config.num_steps, lookup)
      inputs = [tf.squeeze(tensor, [1]) for tensor in inputs0]
  ...
  def __init__(self, config):
    ...
    # tf.concat produce a matrix of batch_size * (num_steps*len(self.vocab)) matrix
    # reshape output's sequence 'row is per batch, num_steps concatenated
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
        ...
        for onestep in inputs:
            newstep = tf.nn.dropout(onestep, self.dropout_placeholder) 
            if len(rnn_outputs) > 0:
                top_h = rnn_outputs[-1]
            else:
                top_h = self.initial_state
            new_top_h = tf.nn.dropout(top_h, self.dropout_placeholder)
            output = tf.sigmoid(tf.matmul(newstep, self.I) + tf.matmul(new_top_h, self.H) + self.b1)
  ...
  for i in xrange(stop_length):
    ...
    for t in real_tokens:
        feed_dict = {
                ...
                model.dropout_placeholder: np.float(1.),
